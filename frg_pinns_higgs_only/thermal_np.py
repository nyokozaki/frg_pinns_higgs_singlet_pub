...
def scalar_masses_AE(rho_phys, t_phys):
    ...
    u_rho_b = (muH2_t + 2.0 * lam_t * rho_phys) / C

    mG2_b = u_rho_b
    mH2_b = u_rho_b + 2.0 * rho_phys * u_rhorho

    # 2. Effective Masses (with Pi)
    u_rho_e = (muH2_eff + 2.0 * lam_t * rho_phys) / C

    mG2_e = u_rho_e
    mH2_e = u_rho_e + 2.0 * rho_phys * u_rhorho
...
def u_thermal_finiteT_AE(t_phys, rho_phys):
    ...
    ring_H = 1.0 * (get_m3(mH2_e) - get_m3(mH2_b))
...
def u_pert(t_phys, rho_phys):
    g1_t, g2_t, yt_t, lam_t, mhsq_t = get_running_couplings(t_phys)
    kt = k_IR * np.exp(-t_range + t_phys)
...
def u_seed_finiteT(t_phys, rho_phys):
    ...
    u_th_offset = u_thermal_finiteT_AE(t_phys, rho_zero)
...
def plot():
    ...
    for i, t_val in enumerate(t_vals):
        ...
        u_pert_vals = u_pert(t_np, rho_np)
        u_pert_offset = u_pert(t_np, np.zeros_like(rho_np))
        u_pert_vals = u_pert_vals - u_pert_offset

        u_finiteT_vals = u_seed_finiteT(t_np, rho_np)
    ...
    plt.title(f"Thermal Potential vs Zero-T Potential \n*RG-improved mass & Arnold-Espinosa resummation*")
    plt.xlabel("$\\rho$")
