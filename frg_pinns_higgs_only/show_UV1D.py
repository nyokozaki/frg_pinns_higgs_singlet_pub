...
def make_model_for_range(model_path, t0, t1, rho0, rho1):
...
def eval_model_slice(model, t_val, rho_np):
    ...
    with torch.no_grad():
        u_pred = model(t_in, rho_in)[0].detach().cpu().numpy().flatten()

    with torch.no_grad():
        u_tree = u_tree_exact(t_t, rho_t).detach().cpu().numpy().flatten()

    return u_tree, u_pred


def eval_model_point(model, t_val, rho_val):
    t_t = my_to([[t_val]])
    rho_t = my_to([[rho_val]])
    ...
    with torch.no_grad():
        u_pred = model(t_in, rho_in)[0].item()
...
def stitched_slice_matched(
    ...
    for irho in range(n_rho_blocks):
        ...
        file_model = f"./data_UV/model_higgs_only_u_rho{irho}_block{block_id}.pt"

        model = make_model_for_range(
            file_model,
            t0, t1, rho_start, rho_end
        )
...
def plot_u():
    t_list = [0.0, -0.5, -1.0, -1.5, -2.0]

    fig, axes = plt.subplots(2, 3, figsize=(17, 9), sharex=True, sharey=False)
    axes = axes.flatten()
    ...
    for i, t_val in enumerate(t_list):
        rho_all, u_tree_all, u_pred_all, shifts = stitched_slice_matched(
            t_val=t_val,
            n_per_block=200
        )
        ...
        u_pert_vals = thermal.u_pert(t_np, rho_np)
        u_pert_offset = thermal.u_pert(t_np, np.zeros_like(rho_np))
        u_pert_vals = u_pert_vals - u_pert_offset

        u_finiteT_vals = thermal.u_seed_finiteT(t_np, rho_np)
        ...
        ax.set_title(f't={t_val:.2f}, block={block_id}')
        ax.set_xlabel('rho')
...
def plot_training_histories_all_rho():
    ...
    for irho in range(n_rho_blocks):
        file_hist_1 = f"./data_UV/history_higgs_only_u_rho{irho}_block1.npy"
        file_hist_2 = f"./data_UV/history_higgs_only_u_rho{irho}_block2.npy"
...
def load_models_blockwise(rho0, rho1):
    ...
    for irho in range(n_rho_blocks):
        ...
        file1 = f"./data_UV/model_higgs_only_u_rho{irho}_block1.pt"
        file2 = f"./data_UV/model_higgs_only_u_rho{irho}_block2.pt"

        models_block1[irho] = make_model_for_range(
            file1,
            -1.0, 0.0, rho_start, rho_end
        )
        models_block2[irho] = make_model_for_range(
            file2,
            -2.0, -1.0, rho_start, rho_end
        )
...
def residual_check_blockwise(
    ...
    for it, t_val in enumerate(t_vals):
        ...
        for ir, rho_val in enumerate(rho_vals):
            ...
            if finite_T:
                ...
                factorH = coth(EH / (2.0 * tau_t), eps=0.0)
    ...
    ax.set_yscale('log')
    ax.set_title('relative residual vs t')
    ax.set_xlabel('t')
...
def plot_residual():
    ...
    _dx = 0.350
    _xtot = 0.350*5
    models_b1, models_b2 = load_models_blockwise(0.0, _xtot)

    residual_check_blockwise(
        models_b1, models_b2,
        t0=-2.0, t1=0.0,
        rho0=0.0, rho1=_xtot,
        invert_t_axis=False
