...
def train_one_block_joint(
    ...
    def compute_losses():
        ...
        if model_left is not None and sigma_ov_min is not None and sigma_ov_max is not None:
            L_overlap = loss_interface_overlap(
                model_left, model, rho_min, rho_max, t_min, t_max, sigma_ov_min, sigma_ov_max, n_interface
            )
        else:
...
def train_sigma_window_joint(
    ...
    has_left_interface = (sigma_ov_min is not None) and all(m is not None for m in model_left_blocks)
    ...
    for iblock, ((t0, t1), n_epochs, w_bc_now) in enumerate(
        ...
        file_model = f"./data_sigma/model_higgs_singlet_u_sigma{isigma}_block{iblock}.pt"
        file_hist = f"./data_sigma/history_higgs_singlet_u_sigma{isigma}_block{iblock}.npy"
        file_ckpt = f"./data_sigma/checkpoint_higgs_singlet_u_sigma{isigma}_block{iblock}.pt"
...
def train(n_epochs=1000, sigma_blocks=None):
    ...
    if first_sigma > 0:
        ...
        prev_sigma_start = prev_sigma * d_sigma
        if prev_sigma < n_sigma_blocks - 1:
            prev_sigma_end = (prev_sigma + 1) * d_sigma + sigma_overlap
        else:
    ...
    for isigma in sigma_blocks:
        ...
        if isigma < n_sigma_blocks - 1:
            sigma_end = (isigma + 1) * d_sigma + sigma_overlap
        else:
            sigma_end = (isigma + 1) * d_sigma

        if isigma > 0:
            sigma_ov_min = sigma_start
            sigma_ov_max = sigma_start + sigma_overlap
        else:
            sigma_ov_min = None
            sigma_ov_max = None

        print(f"\n===== sigma block {isigma}: [{sigma_start:.3f}, {sigma_end:.3f}] =====")
        if sigma_ov_min is not None:
            print(f"      Overlap with previous: [{sigma_ov_min:.3f}, {sigma_ov_max:.3f}]")

        hist_list, trained_models = train_sigma_window_joint(
            isigma=isigma,
